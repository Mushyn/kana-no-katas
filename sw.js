const CACHE_NAME = 'kana-no-katas-v45';
const ASSETS = [
  './',
  'index.html',
  'style.css',
...
// Fetch : cache-first, fallback réseau, et filet de sécurité hors-ligne
  ...
  event.respondWith(
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(response => {
        ...
        // Ni cache, ni réseau (vraiment hors connexion) : éviter l'écran
        // d'erreur natif du navigateur. Pour une navigation de page, on
        // retombe sur l'écran principal (déjà en cache) plutôt que rien.
